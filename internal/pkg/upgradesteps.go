...
func createReplaceSteps(fileName string, variables map[string]interface{}) ([]step.Step, error) {
	...
	expectedFileContentBB := expectedFileContent.Bytes()
	...
	if strings.HasSuffix(replace.Target, ".go") {
		r1 := regexp.MustCompile(`// Code generated by github\.com/99designs/gqlgen version .*\n`)
		expectedFileContentBB = r1.ReplaceAll(expectedFileContentBB, []byte(" "))
		actualFileContent = r1.ReplaceAll(actualFileContent, []byte(""))

		r2 := regexp.MustCompile(`[\n\r\s]+`)
		expectedFileContentBB = r2.ReplaceAll(expectedFileContentBB, []byte(" "))
		actualFileContent = r2.ReplaceAll(actualFileContent, []byte(" "))
	}

	steps := []step.Step{}
	if !bytes.Equal(expectedFileContentBB, actualFileContent) {
		steps = append(
			steps,
